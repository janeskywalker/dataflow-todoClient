...
export const retryDelete = {

    local(data, state) {

        //const newState = Object.assign({}, state, {retryCount: data.retryCount, error: data.error})
        const newState = Object.assign({}, state, 
            {error: {type: Errors.DELETE, data: null}, 
            retryCount: state.retryCount + 1})
        
        console.log('state after updated delete error:', newState)
        return newState
    },
    ...
    remote(data, state, messages) {
            ...
            setTimeout(() => {messages(Actions.DELETE, data)}, waitTime)
    }
}